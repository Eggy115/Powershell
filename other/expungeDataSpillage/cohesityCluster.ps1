...
class CohesityCluster {
    ...
    CohesityCluster([string]$server, [string]$username, [string]$domain, [switch]$updatePassword=$false, [switch] $quiet=$false){
        #$this.authenticate($server,$username,$domain,$updatePassword,$quiet)
        ...
        try {
            ...
            $auth = Invoke-RestMethod -Method Post -Uri $url  -Header $this.HEADER -Body $(
                ConvertTo-Json @{
                    'domain' = $domain; 
                    'password' = (getpwd $server $username $domain $updatepw); 
                    'username' = $username
   ...
   <#  [void]authenticate([string]$server, [string]$username, [string]$domain, [switch]$updatePassword=$false, [switch] $quiet=$false){
...
#$myCluster = [CohesityCluster]::new('bseltzve01','admin','local', $false, $false)
#$myCluster = [CohesityCluster]::new('bseltzve01','admin')
#$myCluster.get('protectionJobs') | ft
