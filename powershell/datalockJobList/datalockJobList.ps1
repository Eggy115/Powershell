...
$csvFile = "datalockJobList_$($cluster.name)_$fileDate.csv"
$htmlFile = "datalockJobList_$($cluster.name)_$fileDate.html"
...
"Job Name,Environment,Policy,Data Lock,Storage Domain,Encrypted" | Out-File -FilePath $csvFile
...
foreach($job in $jobs | Sort-Object -Property name){
    ...
    if($local){
        ...
        $html += "<tr><td>{0}</td>
                ...
                <td>{3}</td>
                <td>{4}</td>
                <td>{5}</td></tr>" -f $jobName, $jobType, $policyName, $datalock, $storageDomain.name, $encrypted
        "{0},{1},{2},{3},{4},{5}" -f $jobName, $jobType, $policyName, $datalock, $storageDomain.name, $encrypted | Out-File -FilePath $csvFile -Append
    }
}
...
Write-Host "also as csv file $csvFile`n"
