...
# CSV output
$csvFileName = "protectionGroupScheduleReport_$($cluster.name)_$dateString.csv"
"Object Name,Next Scheduled Time,Next Run Type,Job Name,Registered Source,Environment,Organization Name" | Out-File -FilePath $csvFileName

# HTML output
$htmlFileName = "protectionGroupScheduleReport_$($cluster.name)_$dateString.html"

$title = "Protection Group Schedule"
...
foreach($job in $report){
    ...
    foreach($object in $job.leafSources){
        ...
        $Global:html += '<tr style="border: 1px solid {7} background-color: {7}">
            ...
            <td>{5}</td>
            <td>{6}</td>
            </tr>' -f $objectName, $nextStart, $nextRunType, $jobName, $source, $objectType, $orgName, $Global:trColor
    }
}
