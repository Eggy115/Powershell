...
# data protection
api get protectionSources | ConvertTo-Json -Depth 99 | Out-File -FilePath (Join-Path -Path $configPath -ChildPath 'sources.json')
api get protectionPolicies?_includeTenantInfo=true | ConvertTo-Json -Depth 99 | Out-File -FilePath (Join-Path -Path $configPath -ChildPath 'policies.json')
api get protectionJobs?allUnderHierarchy=true | ConvertTo-Json -Depth 99 | Out-File -FilePath (Join-Path -Path $configPath -ChildPath 'jobs.json')
