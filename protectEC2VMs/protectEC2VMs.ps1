...
param (
    ...
    [Parameter()][ValidateSet('kAgent', 'kNative', 'kSnapshotManager')][string]$protectionType = 'kNative',
...
if(! $job){
    ...
    $newJob = $false
    $awsParam = $awsParamName[$job.awsParams.protectionType]
    if($job.awsParams.$awsParam.sourceId -ne $sourceId){
...
foreach($vm in $vmnames){
    ...
    if($vmid){
        ...
        $existingObject = $job.awsParams.$awsParam.objects | Where-Object {$_.id -eq $vmid}
        if(! $existingObject){
            $job.awsParams.$awsParam.objects = @($job.awsParams.$awsParam.objects + @{"id" = $vmid; "name" = $vm})
        }
    }else{
