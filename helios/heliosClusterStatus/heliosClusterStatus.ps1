### process commandline arguments
[CmdletBinding()]
param (
   [Parameter()][string]$vip='helios.cohesity.com', #the cluster to connect to (DNS name or IP)
   [Parameter()][string]$username='helios', #username (local or AD)
   [Parameter()][string]$domain = 'local', #local or AD domain
   [Parameter()][ValidateSet('MiB','GiB','TiB')][string]$unit = 'TiB',
   [Parameter()][string]$smtpServer, #outbound smtp server '192.168.1.95'
   [Parameter()][string]$smtpPort = 25, #outbound smtp port
   [Parameter()][array]$sendTo, #send to address
   [Parameter()][string]$sendFrom #send from address
)

### source the cohesity-api helper code
. $(Join-Path -Path $PSScriptRoot -ChildPath cohesity-api.ps1)

### authenticate
apiauth -vip $vip -username $username -domain $domain

$conversion = @{'MiB' = 1024 * 1024; 'GiB' = 1024 * 1024 * 1024; 'TiB' = 1024 * 1024 * 1024 * 1024}
function toUnits($val){
    return "{0:n1}" -f ($val/($conversion[$unit]))
}

$date = (get-date).ToString()
$title = "Helios Cluster Status"

$html = '<html>
<head>
    <style>
        p {
            color: #555555;
            font-family:Arial, Helvetica, sans-serif;
        }
        span {
            color: #555555;
            font-family:Arial, Helvetica, sans-serif;
        }
        

        table {
            font-family: Arial, Helvetica, sans-serif;
            color: #333333;
            font-size: 0.75em;
            border-collapse: collapse;
            width: 100%;
        }

        tr {
            border: 1px solid #F1F1F1;
        }

        td,
        th {
            text-align: left;
            padding: 6px;
        }

        tr:nth-child(even) {
            background-color: #F1F1F1;
        }
    </style>
</head>
<body>
    
    <div style="margin:15px;">
        <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAARgAAAAoCAMAAAASXRWnAAAC8VBMVE
        " style="width:180px">
        <p style="margin-top: 15px; margin-bottom: 15px;">
            <span style="font-size:1.3em;">'

$html += $title
$html += '</span>
<span style="font-size:1em; text-align: right; padding-top: 8px; padding-right: 2px; float: right;">'
$html += $date
$html += '</span>
</p>
<table>
<tr>
    <th>Cluster Name</th>
    <th>Cluster Health</th>
    <th>Logical Data ({0})</th>
    <th>Physical Used ({0})</th>
    <th>Physical Free ({0})</th>
</tr>' -f $unit

"Gathering cluster info..."
foreach($cluster in heliosClusters){
    
    heliosCluster $cluster
    "  $($cluster.name.ToUpper())"

    $clusterStatus = api get /nexus/cluster/status
    $healingStatus = $clusterStatus.healingStatus

    $cluster = api get cluster?fetchStats=true
    $logicalUsage = $cluster.stats.logicalStats.totalLogicalUsageBytes
    $physicalCapacity = $cluster.stats.localUsagePerfStats.physicalCapacityBytes
    $physicalUsed = $cluster.stats.localUsagePerfStats.totalPhysicalUsageBytes
    $physicalFree = $physicalCapacity - $physicalUsed

    $html += "<tr>
        <td>{0}</td>
        <td>{1}</td>
        <td>{2:n0} {5}</td>
        <td>{3} {5}</td>
        <td>{4} {5}</td>
    </tr>" -f $cluster.name.ToUpper(), $healingStatus.ToLower(), ([math]::Round((toUnits $logicalUsage),0)), (toUnits $physicalUsed), (toUnits $physicalFree), $unit
}


$html += '</body></html>'
$outFile = $(Join-Path -Path $PSScriptRoot -ChildPath "heliosClusterStatus-$((get-date).tostring('yyyy-MM-dd')).html")
$html | Out-File -FilePath $outFile
if($smtpServer -and $sendTo -and $sendFrom){
    foreach($toaddr in $sendTo){
        "Sending report to $toaddr"
        Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject $title -BodyAsHtml $html -WarningAction SilentlyContinue
    }
}

