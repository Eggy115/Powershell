...
foreach($vm in $vmNames){
    ...
    if($snapshotId){
        ...
        $restores += $snapshotId
        $restoreParams.acropolisParams.objects = @($restoreParams.acropolisParams.objects +@{
...
if($restoreParams.acropolisParams.objects.Count -gt 0){
    $recovery = api post -v2 data-protect/recoveries $restoreParams
    $recoverId = $recovery.id
    if($wait){
    ...
    Write-Host "No recoveries to perform" -ForegroundColor Yellow
    exit
}
