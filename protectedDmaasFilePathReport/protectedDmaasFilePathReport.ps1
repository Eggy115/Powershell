...
[CmdletBinding()]
param (
    [Parameter()][string]$username = 'DMaaS',   # optional username
    [Parameter()][string]$password,             # optional password
    [Parameter()][switch]$noPrompt              # optional do not prompt for password
)
...
$csvFileName = "protectedFilePaths-$dateString.csv"
$Global:htmlFileName = "protectedFilePaths-$dateString.html"

$title = "Protected File Paths Report"
...
foreach($region in ($regions.tenantRegionInfoList.regionId | Sort-Object)){
    Write-Host "Gathering protected sources in region $region...`n"
    ...
    foreach($source in ($protectedSources | Sort-Object -Property name)){
        ...
        if($obj.objects[0].PSObject.Properties['objectBackupConfiguration']){
            ...
            $includedPath = $obj.objects[0].objectBackupConfiguration.physicalParams.fileObjectProtectionTypeParams.objects[0].filePaths[0].includedPath
